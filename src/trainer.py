...
from src.model.model import NRMS
...
class Trainer(BaseTrainer):
    ...
    def train(self):
        ...
        news_encoder = NewsEncoder.from_pretrained(args.pretrained_embedding, config=config,
                                                   apply_reduce_dim=args.apply_reduce_dim,
                                                   use_cls_embed=args.use_cls_embed, use_sapo=args.use_sapo,
                                                   ...
                                                   category_embed=category_embed, num_category=len(self._category2id),
                                                   num_heads=args.num_heads, attn_dropout=args.attn_dropout,
                                                   combine_type=args.combine_type)
        user_encoder = UserEncoder(news_encoder=news_encoder, num_heads=args.num_heads, attn_dropout=args.attn_dropout,
                                   query_dim=args.query_dim)
        model = NRMS(user_encoder=user_encoder, news_encoder=news_encoder)
    ...
    def _forward_step(model, batch):
        logits = model(title=batch['title'], title_mask=batch['title_mask'], his_title=batch['his_title'],
                       his_title_mask=batch['his_title_mask'], his_mask=batch['his_mask'], sapo=batch['sapo'],
                       sapo_mask=batch['sapo_mask'], his_sapo=batch['his_sapo'], his_sapo_mask=batch['his_sapo_mask'],
                       category=batch['category'], his_category=batch['his_category'])
