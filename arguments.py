...
def _add_model_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--num_heads', type=int, help='Number of parallel attention heads in Multi-head attention')
    parser.add_argument('--combine_type', type=str, choices=['linear', 'attn_pooling'])
    parser.add_argument('--attn_dropout', type=float, help='Dropout probability in the multi-head attention network')
